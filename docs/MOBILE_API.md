...
Возвращает текущий сохранённый календарь. Если данных ещё нет — возвращает пустой словарь.
...
`updatedAt` используется клиентом для разрешения конфликтов при офлайн-редактировании:

1. Клиент хранит локально `days` + `localUpdatedAt` (время последнего изменения на устройстве)
2. При выходе в онлайн: `GET /api/v1/me/calendar` → получить `{days, updatedAt}` с сервера
3. Если `localUpdatedAt > updatedAt` → отправить локальные данные через `PUT`
4. Если `updatedAt > localUpdatedAt` → обновить локальные данные из ответа сервера
5. Если равны → ничего не делать
