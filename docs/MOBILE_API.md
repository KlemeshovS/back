...
Позволяет смотреть календарь другого пользователя при условии взаимной подписки (дружбы). Проверка выполняется на бэке — фронт просто передаёт `userId`.
...
Возвращает календарь пользователя по его ID. Доступно только при **взаимной подписке** (оба подписаны друг на друга).

Если запросить собственный `userId` — поведение идентично `GET /me/calendar`.
...
Формат `days` и `updatedAt` — идентичен `GET /me/calendar`. Семантика эпохи также применяется: если у друга нет сохранённых данных, `updatedAt = "1970-01-01T00:00:00Z"`.

### Коды ошибок

- `NOT_FRIENDS` (`403`) — нет взаимной подписки с этим пользователем
- `USER_NOT_FOUND` (`404`) — пользователь не найден

### Типичный сценарий

1. Получить список друзей: `GET /api/v1/follows/friends` → список с `userId`
2. При открытии профиля друга вызвать: `GET /api/v1/users/{userId}/calendar`
3. Если ответ `403 NOT_FRIENDS` — подписка больше не взаимная (один отписался), скрыть календарь

### Важно

- Взаимность проверяется на бэке, не на фронте — не нужно самому проверять `isMutual` перед вызовом
- Если пользователь отписался после того как ты открыл его профиль — следующий запрос вернёт `403`
