    ...
    this.docsSections = [
      ...
      },
      {
        ...
        cards: [
          ...
          {
            paragraphs: [
              "Список админов читается через GET /admin/admin-users.",
              "Создание admin идет через POST /admin/admin-users и доступно только owner.",
              "Редактирование admin идет через PATCH /admin/admin-users/{adminId}.",
              "Удаление admin идет через DELETE /admin/admin-users/{adminId}.",
              "В UI admins flow тоже собран через context menu с 3 точками: редактирование открывается в modal, удаление требует подтверждения, а создание вынесено в шапку таблицы.",
            ],
          },
        ],
      },
